...
En el modelado de figuras complejas, es habitual trabajar con componentes independientes. Un busto, por ejemplo, puede tener los ojos modelados como objetos separados, lo que facilita enormemente la animación y la gestión de la escena durante el proceso de diseño. El busto que utilizamos como ejemplo en este tutorial fue modelado por [Nicole Aguilar](https://www.instagram.com/bynicole.30?igsh=MXhyaHZ4dW01M21nbg==) en Blender, demostrando cómo una estructura fragmentada en objetos independientes permite una gestión eficiente de componentes complejos. Sin embargo, esta fragmentación del modelo presenta un problema crítico cuando se prepara para impresión tridimensional: el archivo resultante contiene múltiples objetos desconectados que la impresora no reconoce como una pieza única y cohesionada. La siguiente imagen ilustra esta problemática. En Chitubox se observan claramente dos estados de la misma pieza: a la izquierda, el modelo importado sin unir, donde se aprecian las diferentes partes desconectadas; a la derecha, la misma pieza tras aplicar la fusión de meshes en Blender, mostrando una geometría completamente integrada que la impresora reconoce como una sola entidad.  
